...
A Mario-style 2D platformer built from scratch in Python using **Pygame**, featuring custom physics, double jump, a side-scrolling parallax background, enemy AI, power-ups, and multi-level progression.
...
- **Double jump** — a second mid-air jump for more expressive platforming
- **Side-scrolling camera** that follows the player and clamps to level bounds
- **Layered parallax background** — real sprite-based sky, hills, clouds, and trees scrolling at different depths
- **Enemy AI** — patrolling zombie enemies with stomp-to-defeat and side-collision damage mechanics
- **Power-up system** — collect items to grow and take an extra hit before dying
- **Sprite-based animation** — idle, walk, jump, fall, duck, and hurt states for both player and enemies
...
├── player.py            # Player class — movement, animation, double jump, power-ups
├── enemy.py               # Enemy class — patrol AI, stomp/hit detection
├── camera.py                # Side-scrolling camera logic
├── background.py              # Layered parallax background (sky, hills, clouds, trees)
...
└── assets/
    ...
    ├── enemy/                            # Zombie enemy sprite frames
    └── background/                         # Sky, hills, cloud, and tree sprites
```
...
- **`Player`** handles its own physics, input, animation state, double jump tracking, and power-up/damage logic
- **`Enemy`** manages patrol movement, wall/platform detection, and squash animation using real zombie sprites
- **`Background`** renders a multi-layer parallax scene (sky, far hills, near hills, clouds, trees), each scrolling at its own speed for a sense of depth
...
Character, enemy, and background sprites from [Kenney.nl](https://kenney.nl) (CC0 licensed, free for commercial and personal use).
...
- [ ] More levels
- [ ] Checkpoint system
- [ ] Mushroom-style visual power-up sprite
